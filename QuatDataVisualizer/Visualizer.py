...
class DataVisualizer:
    ...
    def performRotation(self, rotation):
        ...
        point_dict_copy["bottom_right"] = rotation.apply(
            point_dict_copy["bottom_right"]
        )
    ...
    def animationCallback(self, i):
        ...
        try:
            ...
            gt_rot = Rotation.from_quat(
                [gt_quat["x"], gt_quat["y"], gt_quat["z"], gt_quat["w"]]
            )
            est_rot = Rotation.from_quat(
                [est_quat["x"], est_quat["y"], est_quat["z"], est_quat["w"]]
            )
        except KeyError:
            return
        ...
        self.gt_plot.plot(
            [0, gt_x[0]], [0, gt_x[1]], [0, gt_x[2]]
        )  # local frame x axis

        self.gt_plot.plot(
            [0, gt_y[0]], [0, gt_y[1]], [0, gt_y[2]]
        )  # local frame y axis

        self.gt_plot.plot(
            [0, gt_z[0]], [0, gt_z[1]], [0, gt_z[2]]
        )  # local frame z axis
        ...
        self.est_plot.plot(
            [0, est_x[0]], [0, est_x[1]], [0, est_x[2]]
        )  # local frame x axis

        self.est_plot.plot(
            [0, est_y[0]], [0, est_y[1]], [0, est_y[2]]
        )  # local frame y axis
        
        self.est_plot.plot(
            [0, est_z[0]], [0, est_z[1]], [0, est_z[2]]
        )  # local frame z axis
    ...
    def runViz(self):
        ...
        self.anim = animation.FuncAnimation(
            self.fig, self.animationCallback, interval=1
        )
        plt.show()
        return self.anim
